 ...
 *
 * @param {AdapterInstance} adapter Adapter-Instanz dieses Autodarts-Adapters
 */
...
async function updateThrow(adapter, lastDart) {
	// @ts-expect-error - calcScore is defined in the Autodarts adapter class
	const score = adapter.calcScore(lastDart);
	const segment = lastDart?.segment?.number || 0;

	// Score-Range aus Adapter übernehmen
	// @ts-expect-error - tripleMinScoreRuntime is defined in the Autodarts adapter class
	let minScore = adapter.tripleMinScoreRuntime;
	// @ts-expect-error - tripleMaxScoreRuntime is defined in the Autodarts adapter class
	...
	// @ts-expect-error - tripleResetTimer is defined in the Autodarts adapter class
	...
	// @ts-expect-error - bullResetTimer is defined in the Autodarts adapter class
	...
	// @ts-expect-error - triggerResetMsRuntime is defined in the Autodarts adapter class
